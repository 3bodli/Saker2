...
const db = new sqlite3.Database('./data/database.sqlite', (err) => {
    if (err) {
        ...
        console.log('✅ تم إنشاء/فتح قاعدة البيانات');
        ...
        // ========== إضافة مستخدم admin إذا ما كان موجود ==========
        db.get("SELECT * FROM users WHERE username = 'admin'", [], (err, row) => {
            if (err) {
                console.error('❌ خطأ في البحث عن admin:', err.message);
                return;
            }
            
            if (!row) {
                const hashedPassword = bcrypt.hashSync('admin123', 10);
                db.run(`INSERT INTO users (username, password, role, created_at) 
                        VALUES (?, ?, ?, ?)`, 
                    ['admin', hashedPassword, 'admin', new Date().toISOString()],
                    function(err) {
                        if (err) {
                            console.error('❌ خطأ في إضافة admin:', err.message);
                        } else {
                ...
                console.log('✅ حساب admin موجود مسبقاً');
            }
        });
...
app.post('/api/register', (req, res) => {
    ...
    db.get("SELECT * FROM users WHERE username = ?", [username], (err, row) => {
        ...
        db.run(`INSERT INTO users (username, password, role, created_at) 
                VALUES (?, ?, ?, ?)`,
            [username, hashedPassword, 'user', new Date().toISOString()],
            function(err) {
                if (err) {
                    return res.status(500).json({ success: false, error: err.message });
                }
...
app.post('/api/create-transfer', verifyToken, (req, res) => {
    ...
    db.run(`INSERT INTO transfers (type, sender_name, note, amount, final_amount, status, created_at, user_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        ['to_me', senderName, note || '', amount, finalAmount, 'pending', new Date().toISOString(), req.user.username],
        function(err) {
            if (err) {
                return res.status(500).json({ success: false, error: err.message });
            }
            res.json({ success: true, transferId: this.lastID, finalAmount });
        }
    );
});
...
app.post('/api/create-company-transfer', verifyToken, (req, res) => {
    ...
    db.run(`INSERT INTO transfers (type, company_name, receiver_name, amount, final_amount, status, created_at, user_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        ['to_company', companyName, receiverName, amount, finalAmount, 'pending', new Date().toISOString(), req.user.username],
        function(err) {
            if (err) {
                return res.status(500).json({ success: false, error: err.message });
            }
...
app.get('/api/admin/transfers', verifyToken, (req, res) => {
    ...
    db.all("SELECT * FROM transfers ORDER BY id DESC", [], (err, transfers) => {
        if (err) {
            return res.status(500).json({ success: false, error: err.message });
        }
...
app.put('/api/admin/update-transfer/:id', verifyToken, (req, res) => {
    ...
    db.run("UPDATE transfers SET status = ? WHERE id = ?", [req.body.status, req.params.id], (err) => {
        if (err) {
            return res.status(500).json({ success: false, error: err.message });
        }
...
app.get('/api/admin/stats', verifyToken, (req, res) => {
    ...
    db.get("SELECT COUNT(*) as total FROM transfers", [], (err, total) => {
        db.get("SELECT COUNT(*) as pending FROM transfers WHERE status = 'pending'", [], (err, pending) => {
            db.get("SELECT SUM(amount) as totalAmount FROM transfers", [], (err, amount) => {
                res.json({ 
                    success: true, 
                    stats: { 
                        total: total.total, 
